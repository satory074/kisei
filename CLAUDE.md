...
- **テストフレームワークは無い**（moshirasu 方式の fail-fast assert スクリプト）。`smoketest.ts` はデータ検証・時刻演算・日跨ぎ・パレート・合成フィクスチャ探索・損益分岐（平文4分岐含む）・実価格上書き・日別料金（validate+fareByDay探索）・出発日くらべ（compareWindow/compareDays、フィクスチャ+実データ）・料金入力グリッド（daygrid のパース/直列化/更新＋buildFareByDay/compareDays への重ね）・戦略グルーピング・実データシナリオ（例示4モードパターン+グループ数5〜15の検証）・URL round-trip。`domtest.ts` は jsdom で boot→検索→**戦略グループ2段カード**描画→出発日くらべバー（エンジン計算との突き合わせ・日クリック・全日同額時のバッジ抑制）→料金入力グリッド（入力→localStorage保存→バー反映→クリア、boot時の復元）→ソート→実価格上書き/クリア（open状態維持含む）→日別料金表示→共有URL復元（`fares=` 含む）。
...
- **出発日くらべ（`daycompare.ts`）**: 検索のたび選択日±3日の7日ぶん `searchRoutes` を回し（実測 0.6ms/回なのでキャッシュ不要）、各日の最安総額＋戦略ラベルをバー表示（`#daybar`、結果ヘッダの上）。純モジュールなので smoketest が直接検証する。ウィンドウは過去日を今日へクランプして常に7日（共有URLの過去日付では選択日が窓外に出るが、日クリックで自己修復）。日クリック＝`set-date` コマンド→`setForm`+`runSearch`（窓の再センタリングはこれだけで成立）。`dayFares` は boot() の state に保持し、set-sort は再検索せず使い回す。「目安」判定は最安行程の `fare.low !== fare.high`（`fareHtml` と同じ規約。calendarFare・実価格上書きは幅を単一値に潰すため）。**全日同額なら最安ハイライトを出さない**（カレンダー未カバー期間・全変動エッジ上書き時のノイズ回避）。URL パラメータは増やしていない（既存 `date=` を再利用）。
- **料金入力グリッド（`daygrid.ts` + render `#daygrid`）**: 出発日くらべの下の `<details>` パネル。行＝変動レグ・列＝7日の表に実勢価格を手入力でき、`engine` の `fareByDay` に重ねて反映する。`buildFareByDay` を第5引数 `userByDate`（edgeId→ISO日付→円）で拡張し、**優先順位 日別手入力 ＞ 全日共通の実価格上書き ＞ 日別カレンダーJSON ＞ 幅**（全日上書きエッジでも、その日に手入力があれば日別が勝つ）。グリッドは `dayFareGrid`（boot() の state）に持ち、**localStorage（`kisei.dayFareGrid.v1`）に永続化**＝URL には載せない（自分の価格調査メモ。`daygrid.ts` の `parseDayGrid`/`serializeDayGrid` は未知エッジ・不正値を捨てる）。行は**主要戦略の代表（最安）ルートの変動レグだけ**（`collectVolatileLegs`）— その他/遠回りルートは空港ループ等の人工的レグ（行きの旅程に「新千歳→伊丹」が混じる）を含むため除外。セルのプレースホルダ＝エンジンの既定値（全日上書き ＞ カレンダー ＞ 目安typical）。入力は change（blur/Enter）で `set-day-fare` 委譲→localStorage保存→`runSearch`。compareDays も `userDayFares` を受け取りバーへ反映。
