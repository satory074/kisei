...
- **テストフレームワークは無い**（moshirasu 方式の fail-fast assert スクリプト）。`smoketest.ts` はデータ検証・時刻演算・日跨ぎ・パレート・合成フィクスチャ探索・損益分岐（平文4分岐含む）・実価格上書き・日別料金（validate+fareByDay探索）・出発日くらべ（compareWindow/compareDays、フィクスチャ+実データ）・戦略グルーピング・実データシナリオ（例示4モードパターン+グループ数5〜15の検証）・URL round-trip。`domtest.ts` は jsdom で boot→検索→**戦略グループ2段カード**描画→出発日くらべバー（エンジン計算との突き合わせ・日クリック・全日同額時のバッジ抑制）→ソート→実価格上書き/クリア（open状態維持含む）→日別料金表示→共有URL復元（`fares=` 含む）。
